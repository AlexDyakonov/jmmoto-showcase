...
func (c *Cat) MeCats(ctx Context) ([]*domain.Cat, error) {
	return c.ListOwnedCats(ctx, ctx.User.ID)
}

func (c *Cat) PatchCat(ctx Context, catID string, cat *domain.PatchCat) (*domain.Cat, error) {
	if err := c.ensureCatOwnedByUser(ctx, catID); err != nil {
		return nil, err
	}
	err := c.catRepo.Patch(ctx, catID, cat)
	if err != nil {
		return nil, fmt.Errorf("failed to patch cat: %w", err)
	}
	return repo.First(c.catRepo.Filter)(ctx, &domain.FilterCat{ID: &catID})
}
